...
namespace FluentModbus.Tests
{
    public class MultiModbusRtuServerTests : IClassFixture<XUnitFixture>
    {
        ...
        public void AddDynamicallyUnits()
        {
            ...
            var registersOne = server.GetHoldingRegisters(1);
            registersOne.SetLittleEndian(registerAddress, (short)1);
            var regValueOne = client.ReadHoldingRegisters<short>(1, registerAddress, 1).ToArray();

            server.AddUnit(2);
            var registersTwo = server.GetHoldingRegisters(2);
            registersTwo.SetLittleEndian(registerAddress, (short)2);
            var regValueTwo = client.ReadHoldingRegisters<short>(2, registerAddress, 1).ToArray();

            server.AddUnit(3);

            var registersThree = server.GetHoldingRegisters(3);

            registersThree.SetLittleEndian(registerAddress, (short)3);
            var regValueThree = client.ReadHoldingRegisters<short>(3, registerAddress, 1).ToArray();
            client.Close();
        ...
        public void SetMidLittleEndianToInt32()
        {
            ...
            var server = new MultiUnitRtuServer(new byte[] { 1, 2 });
            server.Start("COM1");

            var testNumber = 153096;
            var registers = server.GetHoldingRegisters(unitId);
            registers.SetMidLittleEndian<int>(registerAddress, testNumber);

            var client = new ModbusRtuClient();
            client.Connect("COM2");

            var regValueOne = client.ReadHoldingRegisters<int>(unitId, registerAddress, 1).ToArray();
